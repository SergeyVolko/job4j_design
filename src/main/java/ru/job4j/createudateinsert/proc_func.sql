...
--Задание.
create or replace procedure delete_data(doorstep_price integer)
language 'plpgsql'
as $$
    BEGIN
		delete from products where price < doorstep_price;
    END
$$;
call delete_data(40);
...
$$
	declare
		result varchar(50);
    begin
        select into result name from products where name = i_name;
